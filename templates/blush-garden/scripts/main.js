...
// ============================================
function initColorVariationSwitcher() {
    const switcher = document.getElementById('colorVariationSwitcher');
    const switcherBtn = document.getElementById('colorSwitcherBtn');
    const colorOptions = document.getElementById('colorOptions');

    if (!switcher || !switcherBtn || !colorOptions) return;
    ...
    options.forEach(option => {
        option.addEventListener('click', () => {
            ...
            localStorage.setItem('wedding-color-theme', theme);

            // Close dropdown
            switcher.classList.remove('open');

            // Show toast notification
            const themeName = option.querySelector('.color-name').textContent;
            showToast(`Theme changed to ${themeName}`);
        });
    });

    // Load saved theme on init
    const savedTheme = localStorage.getItem('wedding-color-theme');
    if (savedTheme) {
...
// ============================================
// FLORAL DECORATION VISIBILITY
// ============================================
function initFloralDecorationVisibility() {
    const floralDecoration = document.querySelector('.floral-bottom-decoration');
    const scrollChevron = document.getElementById('scrollChevron');

    if (!floralDecoration && !scrollChevron) return;
    ...
    const updateVisibility = () => {
        ...
        if (floralDecoration) {
            floralDecoration.style.opacity = scrollY > threshold ? '0' : '1';
            floralDecoration.style.transform = scrollY > threshold ? 'translateY(50px)' : 'translateY(0)';
            floralDecoration.style.transition = 'opacity 0.5s ease, transform 0.5s ease';
        }

        if (scrollChevron) {
            scrollChevron.style.opacity = scrollY > threshold ? '0' : '0.7';
            scrollChevron.style.transition = 'opacity 0.5s ease';
        }
...
function init() {
    ...
    initImageProtection();
    initColorVariationSwitcher();
    initScrollBodyClass();
    initFloralDecorationVisibility();
