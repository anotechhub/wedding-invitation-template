...
const DOM = {
    ...
    coverLinks: document.querySelectorAll('a[href="#cover"]'),
...
function initOpenInvitation() {
    const openInvitation = () => {
        ...
        // Enable scrolling and hide cover overlay
        document.body.classList.remove('no-scroll');
        DOM.cover.classList.add('is-hidden');
        DOM.cover.setAttribute('aria-hidden', 'true');

        // Restore last scroll position if returning from cover
        if (state.lastScrollY > 0) {
            window.scrollTo({ top: state.lastScrollY, behavior: 'auto' });
        }
    ...
    const returnToCover = (e) => {
        if (e) {
            e.preventDefault();
        }

        if (!state.isInvitationOpened) {
            window.scrollTo({ top: 0, behavior: 'smooth' });
            return;
        }

        state.isInvitationOpened = false;
        state.lastScrollY = window.scrollY;

        // Show cover overlay and lock scroll
        DOM.cover.classList.remove('is-hidden');
        DOM.cover.removeAttribute('aria-hidden');
        document.body.classList.add('no-scroll');
        DOM.cover.scrollTop = 0;
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };
    ...
    // Return to cover when clicking initials or Home link
    if (DOM.navbarBrand) {
        DOM.navbarBrand.addEventListener('click', returnToCover);
    }

    if (DOM.coverLinks.length) {
        DOM.coverLinks.forEach((link) => {
            link.addEventListener('click', returnToCover);
        });
    }
}
